...
O desenvolvimento do diagrama do padrão de projeto _Visitor_ deve envolver as estruturas apresentadas no tópico [Estrutura](#estrutura). O diagrama apresentado na **Figura 1** abaixo se refere à aplicação do padrão _Visitor_ em um caso de uso fictício, relacionado ao tema do trabalho do Grupo 02: UnBrechó.
